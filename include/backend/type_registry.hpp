...
struct type_registry_t {
  ...
  SP<type_t> resolve(const path_t &name);

  SP<type_t> add_builtin(const path_t &name, size_t size, size_t alignment, type_kind_t kind);
  SP<type_t> add_function(SP<type_t> return_type, const std::vector<SP<type_t>> &arguments, SP<type_t> receiver, bool is_var_args);
  SP<type_t> add_struct(const path_t &name, struct_layout_t layout);
  SP<type_t> add_alias(const path_t &name, SP<type_t>, bool is_distinct);
  SP<type_t> add_contract(const path_t &name, const std::map<std::string, SP<type_t>> &requirements);
  ...
  SP<type_t> self_placeholder(const path_t &name);
  SP<type_t> add_template_alias(const path_t &name, SP<type_t>);
